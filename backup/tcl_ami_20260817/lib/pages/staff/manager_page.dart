...
class _ManagerPageState extends State<ManagerPage>
  ...
  void initState() {
    super.initState();
    print('[DEBUG PRINT] manager initState');
  ...
  void didChangeDependencies() async {
    ...
    print('manager didChangeDependencies');
      ...
      children: [
        ...
        Expanded(
          child: GestureDetector(
            onTap: () {
              if (address.sensor.isNotEmpty && address.sensor.startsWith('pir_')) {
  ...
  void onSensorAlertsChange() {
    ...
    sensorService.alerts.forEach((key, value) {
      final existing = addressStore.find(key);
      final isNewPirAlert = value.toString().startsWith('pir_') &&
          (existing == null || existing.sensor.isEmpty);
      addressStore.setSensor(key, value);
      if (isNewPirAlert) {
        // pirの全アイコン(Move/不在/在宅/呼出)共通で通知音を1回再生
        audio.buttonCall();
      }
    });
