...
import 'package:amiapp/services/socket_service.dart';
import 'package:amiapp/widgets/elan_call_menu_widget.dart';
...
class ElanCallPageState extends State<ElanCallPage>
  ...
  void initState() {
    super.initState();
    print('[DEBUG PRINT] elan call initState');
    _initRenderers();
  ...
  void didChangeDependencies() {
    ...
    print('elan call didChangeDependencies');

    if (_init) {
      _init = false;

      if (AppManager.appsettings['CALLSCREENIMAGE'] != '0') {
        Future(() async {
          var filename = 'dlimage.jpg';
      ...
      // AppManager.threewayId = 'room';
      // AppManager.talkId1 = 'cfs8DILS_TV001';
      // AppManager.talkId2 = 'cfs8DILS_TV001';
  ...
  void _close({bool toHome = false}) {
    print('close');
    audio.stopButtonCall();
    AppManager.selectUser = null;
    Navigator.of(context).pop(toHome);
  }
  ...
  void _rejectCall() {
    ...
    context.read<AddressStore>().setCalledUser(null);
  ...
  void _stopAbsenceTimer() {
    if (_absenceTimer != null) {
  ...
  void _receiveRequestPhoto() async {
  ...
  void _receiveShareImage(Uint8List image) async {
    print(image.length);
  ...
  void _savePhotoImage(String directory) async {
  ...
  ImageProvider _bgImage(Size size) {
    // print('_dlImage is null');

    if (size.width > size.height) {
      if (_dlImagel != null) {
        return _dlImagel!;
      }
      return const AssetImage('assets/images/talk/call_bg_landscape.png');
    }
    if (_dlImage != null) {
      return _dlImage!;
    }
    if (size.width < 500) {
      return const AssetImage('assets/images/talk/call_bg_s.png');
    }
    return const AssetImage('assets/images/talk/call_bg.png');
  }
  ...
  void onAppMessage(data) {
    ...
    print('staff talk app message $message');
    if (message == 'call') {
  ...
  void _onOffer(data) {
    // TODO: implement recordConnectId
