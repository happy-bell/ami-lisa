...
class CheckmeRingService extends ChangeNotifier {
  ...
  static const _popupLingerSeconds = 120; // 測定終了(指を外す)から2分でポップアップを閉じる
  static const _noFingerDisableSeconds = 120; // 指なしのまま2分でボタン解除
  ...
  bool get _needsButton => !AppManager.isPiTvLayout;
  ...
  Future<void> _disconnect(String reason, {bool closePopup = false}) async {
    ...
    if (closePopup) {
      ...
      _popupTimer?.cancel();
      _popupTimer = Timer(
        const Duration(seconds: _popupLingerSeconds + 1),
        notifyListeners,
      );
      notifyListeners();
    }
