...
class SocketIOService {
  ...
  void _setSocket() {
    ...
    _socket!.onConnect((_) {
      ...
      delegate?.onConnect();
      Future.delayed(const Duration(milliseconds: 400), flushPendingCall);
    });
    ...
    _socket!.on("app", (data) {
      ...
      if (message == 'call' && data is Map) {
        _pendingCall = Map<String, dynamic>.from(data);
        _pendingCallAt = DateTime.now();
  ...
  /// 再接続直後に、取りこぼした着信を再通知する。
  void flushPendingCall() {
    final pending = _pendingCall;
    final at = _pendingCallAt;
    if (pending == null || at == null) return;
    if (DateTime.now().difference(at) > const Duration(seconds: 35)) {
      _pendingCall = null;
      _pendingCallAt = null;
      return;
    }
    print('socket flush pending call');
    delegate?.onAppMessage(pending);
    delegate2?.onAppMessage(pending);
  }
