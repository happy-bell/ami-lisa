...
class Peer {
  ...
  Future<MediaStream> createStream(media, userScreen) async {
    ...
    // Google TV + USBカメラでは facingMode(user/environment) を付けると
    // デバイス選択に失敗することがあるため、TV時は付けない。
    ...
    if (TvUtil.isTelevision) {
      ...
      if (audioDeviceId != null && audioDeviceId.isNotEmpty) {
        try {
          // getUserMedia の deviceId だけでは Android 入力は切り替わらない。
          // JavaAudioDeviceModule.setPreferredInputDevice が必要。
    ...
    dynamic audioConstraints;
    if (TvUtil.isTelevision) {
      // TV: シンプルな audio:true + 事前の selectAudioInput(USB)
