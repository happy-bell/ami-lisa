...
object UsbMicBridge {
    ...
    fun start(context: Context): Map<String, Any?> {
        val emeet = UsbAudioCapture.findMicDevice(context)
        if (emeet != null && CameraEmeetProfile.matches(emeet.vendorId, emeet.productName)) {
            // 同じUSB複合機でUACを奪うとUVC映像が波打つため、EMEETはHAL音声のみ使う。
            Log.i(TAG, "UsbMicBridge: EMEET detected; skip UAC injection to protect video")
            return mapOf(
                "ok" to true,
                "skipped" to true,
                "reason" to "emeet_uvc",
                "device" to emeet.productName,
            )
        }
        if (halSupportsUsbInput(context)) {
            // 正規経路(AudioRecord)がUSBマイクを使える端末では、
            // UACで奪うと正常動作を壊すため注入しない。
