...
class _CheckmeMonitorPanelState extends State<CheckmeMonitorPanel> {
  ...
  Widget _header(CheckmeProService pro) {
    final state = pro.measuring
        ? '受信中'
        : '受信 -- (${pro.closeRemainSeconds})';
    return Container(
      height: 34,
  ...
  Widget _body(CheckmeProService pro) {
...
class _CheckmeWavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _plotEcg(canvas, ecg, 0, sy, w, mm, visible);
    _plotPleth(canvas, pleth, sy, h, w, mm, visible);
  }
  ...
  void _plotEcg(
    ...
    final samples = _prepare(data, visible);
    ...
    final stepX = mm / _samplesPerSmall;
    final path = Path();
    ...
    canvas.drawPath(path, _wavePaint(_ecg, w, sharp: true));
  }
