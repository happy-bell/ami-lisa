...
class _StaffPageState extends State<StaffPage>
    ...
    final data = await dio
        .post(url,
            data: FormData.fromMap({
              ...
              'user_id': AppManager.myId,
              'type': AppManager.settings['MCSTYPE'],
              'loginAt': loginAt,
              'manager': AppManager.isManager ? '1' : '0'
            }))
        .then((response) {
  ...
  Widget _managerAddressCell(Address address) {
    final Size size = MediaQuery.of(context).size;
    if (_islist) {
      return StaffUtil.managerAddressListCell(size, address,
  ...
  void _openCallStatusPopup() {
    ...
    for (var i = 0; i < addressStore.addressList.length; i++) {
      ...
      if (address.call == 1 || address.called == 1) {
        ...
        _callStatuses.add(
            {'address': address, 'status': 'call', 'managerName': managerName});
      }
    }
    // print(_callStatuses);
    setState(() {});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppManager.selectCode.isNotEmpty
          ? WidgetUtil.appBar(AppManager.selectCode,
              backgroundColor: Colors.black,
  ...
  void onAppMessage(data) async {
    ...
    if (message == 'from_server') {
      ...
      var index = addressStore.findAddress(udid);
      if (index < 0) {
        await _loadAddress();
        if (!mounted) {
          return;
        }
        addressStore = context.read<AddressStore>();
        index = addressStore.findAddress(udid);
      }
      if (index < 0) {
        return;
      }
      ...
      if (addressStore.find(udid) != null) {
        ...
        message == 'safety_check_stop' ||
        message == 'watching_end') {
      if (data["udid"] == null) {
        return;
      }
      if (!mounted) {
