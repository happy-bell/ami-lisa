...
function thinDisplay(data) {
  // 表示だけ約30%間引き（10点中7点）。測定データ自体は変えない。
  if (!data || data.length < 8) return data;
  var out = [];
  for (var i = 0; i < data.length; i++) {
    if (i % 10 < 7) out.push(data[i]);
  }
  return out;
}

// 画面には約5秒分だけ出す。長い配列を横幅に潰すと心拍が詰まり描画が乱れる。
const VIEW_SAMPLES = 625; // 125Hz × 5秒
function forPlot(data) {
  if (!data || data.length < 4) return data;
  var src = data;
  if (src.length > VIEW_SAMPLES) src = src.slice(src.length - VIEW_SAMPLES);
  return thinDisplay(src);
}
