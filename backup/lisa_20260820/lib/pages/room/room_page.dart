...
class _RoomPageState extends State<RoomPage>
  ...
  /// ami-EX: 他アプリ／電源オフから全面化したあとで通話開始する。
  ...
  void _hideSleep() {
    ...
    _sleeptimer?.cancel();
    _sleeptimer = null;
    // ラズパイ版の時計モードは手動入退出のみ。ホームに戻ったあと
    // 10秒で再入場しない（TCLアミの SLEEP 自動復帰は従来どおり）。
    if (!AppManager.isPiTvLayout) {
      _sleeptimer =
  ...
  Future<void> _handleIncomingCall(String udid) async {
    if (TvUtil.isTelevision) {
      ...
      // 電源オフ／視聴中／他アプリ:
      // ami-EX は自動応答ONなら確認なしで繋ぐ。OFFならはい・いいえ。
      // ami-LiSA は従来どおり自動応答ONなら確認なし。
  ...
  Future<void> _receive(String udid) async {
    ...
    if (AppManager.isPiTvLayout) {
      // ラズパイ版はホームに留める。TCL版の「前のアプリへ戻す」は地デジへ飛んで落ちたように見える。
      _piFocusHome();
