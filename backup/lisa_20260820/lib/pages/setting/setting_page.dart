...
import 'setting_checkme_ring_page.dart';
import 'setting_checkme_pro_page.dart';
...
class _SettingPageState extends State<SettingPage> {
  ...
  List<Widget> _listContainers() {
    ...
    if (dispType == '1' && !AppManager.isPiTvLayout) {
      listContainers.add(
        ...
        listContainers.add(
          nextListContainer('ホーム画面', AppManager.tvLayoutLabel(), () async {
            ...
            if (result != null) {
              ...
              if (result == AppManager.tvLayoutPi && mounted) {
    ...
    if (Platform.isAndroid && mode == 'user') {
      listContainers.add(
        nextListContainer('Checkme Ring（BLEテスト）', '', () {
          Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => const SettingCheckmeRingPage(),
          ));
        }),
      );
      listContainers.add(
        nextListContainer('Checkme Pro（BLEテスト）', '', () {
          Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => const SettingCheckmeProPage(),
          ));
        }),
      );
    }
